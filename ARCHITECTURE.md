...
- **批次调度**：`IngestBatchScheduler` 以分阶段闸门推进批次——分析阶段允许 `analyze-concurrency`（默认 2）份并发，写入阶段严格 1 份串行（写入互斥），两阶段闸门独立可重叠
...
- **批次调度**：≥2 份资料以 `ingest_batch` 聚合提交，由 `IngestBatchScheduler` 分阶段闸门推进——分析阶段最多 `llmwiki.ingest.batch.analyze-concurrency`（默认 2）份并发，写入阶段严格 1 份串行，分析闸门与写入闸门独立计数、互不阻塞；分析完成的资料停留在 `awaiting_confirmation`（收件箱待审阅），用户确认后置 `confirmed` 重新入队，调度器按序推进 Phase 2
