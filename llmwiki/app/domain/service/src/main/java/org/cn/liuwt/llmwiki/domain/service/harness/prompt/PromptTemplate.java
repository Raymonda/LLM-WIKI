...
public class PromptTemplate {
    ...
    public static String knowledgeNormalizationPrinciple() {
        ...
        return LANGUAGE_CONSTRAINT + "\n\n" + """
            ...
            2. 【指代明确化】消解代词与指代性称呼，但保留业务角色名词：
               - "如上所述"/"前文提到"/"后面会讲到" → 使用明确的章节引用或 [[页面链接]]
               - "他"/"她"/"它"/"这个"/"那个"等代词 → 替换为具体名称或 [[页面链接]]
               - "这个人"/"那个项目"/"该公司"/"该团队"等指代性称呼 → 在上下文能唯一确定指称时替换为具体名称或 [[页面链接]]；不能确定时保留原样
               - 业务角色名词（管理人、托管人、销售机构、正回购方、投资经理、用户、甲方、乙方等）必须保留原样，严禁替换为具体实体名称
